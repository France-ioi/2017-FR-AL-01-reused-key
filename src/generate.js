var words = [[
"AILE",
"AUTO",
"AVEC",
"BAIN",
"BANC",
"BEAU",
"BIEN",
"BLEU",
"BOIS",
"BORD",
"BOUE",
"BOUT",
"BRAS",
"BRUN",
"CAGE",
"CAMP",
"CAVE",
"CERF",
"CHAT",
"CHER",
"CHEZ",
"CHOU",
"CIEL",
"CINQ",
"CLOU",
"COIN",
"COUP",
"COUR",
"CUBE",
"CUIT",
"DAME",
"DANS",
"DEMI",
"DENT",
"DEUX",
"DIRE",
"DOUX",
"DRAP",
"FACE",
"FAIM",
"FILM",
"FILS",
"FOIN",
"FOIS",
"FOND",
"FORT",
"FOUR",
"GANT",
"GARE",
"GENS",
"GRIS",
"GROS",
"GRUE",
"HAUT",
"HIER",
"HUIT",
"JEAN",
"JOIE",
"JOLI",
"JOUE",
"JOUR",
"JUPE",
"KIWI",
"LAIT",
"LAME",
"LENT",
"LION",
"LIRE",
"LOIN",
"LONG",
"LOUP",
"LUGE",
"LUNE",
"MAIN",
"MARE",
"MARI",
"MIDI",
"MIEL",
"MINE",
"MOIS",
"MORT",
"MOTO",
"MUET",
"NAIN",
"NEUF",
"NOIR",
"NOIX",
"NUIT",
"OGRE",
"OURS",
"PAGE",
"PAIN",
"PAIX",
"PAPA",
"PARC",
"PART",
"PAYS",
"PEAU",
"PEUR",
"PIED",
"PILE",
"PION",
"PLAT",
"PLUS",
"PNEU",
"POLI",
"PONT",
"PORT",
"POUF",
"PRIX",
"PUIS",
"QUAI",
"RIRE",
"ROBE",
"ROND",
"ROUE",
"ROUX",
"SAGE",
"SALE",
"SANG",
"SAUT",
"SCIE",
"SEAU",
"SENS",
"SEPT",
"SEUL",
"SOIF",
"SOIR",
"SOLE",
"SOUS",
"TARD",
"TIGE",
"TOIT",
"TOUR",
"TROP",
"TROU",
"TUBE",
"TUER",
"VASE",
"VENT",
"VERS",
"VERT",
"VIDE",
"VITE",
"VOIR",
"VOIX",
"YEUX",
],
[
"AIDER",
"AIGLE",
"AIMER",
"ALBUM",
"ALLER",
"AMOUR",
"ANGLE",
"ARBRE",
"ASSEZ",
"ASSIS",
"AVANT",
"AVION",
"BAGUE",
"BALAI",
"BALLE",
"BANDE",
"BARBE",
"BARRE",
"BAVER",
"BELLE",
"BIJOU",
"BILLE",
"BISOU",
"BLANC",
"BLOND",
"BOIRE",
"BOSSE",
"BOTTE",
"BOULE",
"BRUIT",
"CADRE",
"CALME",
"CANNE",
"CHAMP",
"CHAUD",
"CHIEN",
"CHOSE",
"CHUTE",
"CLAIR",
"CLOWN",
"COLLE",
"CONTE",
"CORDE",
"CORPS",
"COUDE",
"COURT",
"CRABE",
"CRAIE",
"CREUX",
"CRIER",
"CROIX",
"CRUEL",
"CUIRE",
"CYGNE",
"DANSE",
"DOIGT",
"DROIT",
"ENGIN",
"ENVIE",
"FAIRE",
"FANER",
"FARCE",
"FAUTE",
"FEMME",
"FERME",
"FESSE",
"FILET",
"FILLE",
"FINIR",
"FLEUR",
"FOIRE",
"FREIN",
"FRITE",
"FROID",
"FRONT",
"FRUIT",
"FUMER",
"FUSIL",
"GARER",
"GELER",
"GENOU",
"GLACE",
"GOMME",
"GORGE",
"GRAIN",
"GRAND",
"HABIT",
"HERBE",
"HEURE",
"HIBOU",
"HIVER",
"HOMME",
"HUILE",
"IMAGE",
"JAMBE",
"JAUNE",
"JETER",
"JEUDI",
"JEUNE",
"JOUER",
"JOUET",
"LACER",
"LACET",
"LAINE",
"LAMPE",
"LAPIN",
"LARGE",
"LARME",
"LAVER",
"LEVER",
"LIGNE",
"LINGE",
"LISSE",
"LISTE",
"LITRE",
"LIVRE",
"LOUPE",
"LOURD",
"LUNDI",
"LUTIN",
"MAGIE",
"MAINS",
"MAMAN",
"MARDI",
"MARIN",
"MATIN",
"MELON",
"MICRO",
"MIEUX",
"MINCE",
"MIXER",
"MOINS",
"MONDE",
"MOULE",
"MOYEN",
"NAGER",
"NAPPE",
"NAVET",
"NEIGE",
"NOYAU",
"NUAGE",
"OBJET",
"ODEUR",
"OMBRE",
"ONGLE",
"ORAGE",
"ORDRE",
"OUTIL",
"PAIRE",
"PANDA",
"PANNE",
"PATTE",
"PAYER",
"PELLE",
"PENTE",
"PERLE",
"PESER",
"PETIT",
"PHOTO",
"PIEDS",
"PLACE",
"PLAGE",
"PLEIN",
"PLIER",
"PLUIE",
"PLUME",
"POCHE",
"POING",
"POINT",
"POIRE",
"POMME",
"POMPE",
"PONEY",
"PORTE",
"POSER",
"POSTE",
"POUCE",
"POULE",
"PRUNE",
"PUNIR",
"QUEUE",
"RADIO",
"RADIS",
"RAMER",
"RAMPE",
"RASER",
"RATER",
"RAYON",
"REINE",
"REPAS",
"RICHE",
"ROUGE",
"ROUTE",
"RUBAN",
"SABLE",
"SALIR",
"SALLE",
"SAPIN",
"SAVON",
"SIGNE",
"SINGE",
"SOUPE",
"SOURD",
"SPORT",
"STYLO",
"SUCER",
"SUCRE",
"TABLE",
"TACHE",
"TAIRE",
"TALON",
"TAPER",
"TAPIS",
"TARTE",
"TASSE",
"TAUPE",
"TEMPS",
"TENIR",
"TENTE",
"TERRE",
"TIGRE",
"TIRER",
"TISSU",
"TITRE",
"TORDU",
"TRAIN",
"TRAIT",
"TRIER",
"TROIS",
"TROUS",
"TUYAU",
"USINE",
"UTILE",
"VACHE",
"VAGUE",
"VENIR",
"VERRE",
"VESTE",
"VIDER",
"VIEUX",
"VILLE",
"VITRE",
"VIVRE",
"VOILE",
"VOLER",
"VOLET",
"WAGON",
],
[
"ABSENT",
"ADROIT",
"ADULTE",
"AGACER",
"AGITER",
"AGNEAU",
"AMENER",
"ANANAS",
"ANCIEN",
"ANIMAL",
"ANORAK",
"ARGENT",
"AUTANT",
"AUTOUR",
"AVALER",
"BAGAGE",
"BAISER",
"BALCON",
"BALLON",
"BANANE",
"BARQUE",
"BARRER",
"BASSIN",
"BATEAU",
"BATTRE",
"BAVOIR",
"BERCER",
"BEURRE",
"BILLET",
"BONBON",
"BONDIR",
"BONNET",
"BOSSER",
"BOTTES",
"BOUCHE",
"BOUDER",
"BOUGER",
"BOUTON",
"BROSSE",
"BULLES",
"BUREAU",
"CABANE",
"CACHER",
"CADEAU",
"CAHIER",
"CAISSE",
"CAMION",
"CAMPER",
"CANARD",
"CARNET",
"CARTON",
"CASIER",
"CASQUE",
"CASSER",
"CERISE",
"CHAISE",
"CHANCE",
"CHEMIN",
"CHEVAL",
"CHEVEU",
"CIRQUE",
"CITRON",
"CLASSE",
"CLOCHE",
"COCHON",
"COFFRE",
"COGNER",
"COLLER",
"CONTRE",
"COPAIN",
"COPIER",
"COQUET",
"COQUIN",
"COUDRE",
"COUPER",
"COURIR",
"COURSE",
"COUSIN",
"COUVER",
"CRAYON",
"CUISSE",
"DANGER",
"DANSER",
"DEBOUT",
"DEDANS",
"DEHORS",
"DEMAIN",
"DESSIN",
"DESSUS",
"DEVANT",
"DEVOIR",
"DINDON",
"DOIGTS",
"DOMINO",
"DONNER",
"DORMIR",
"DOUCHE",
"EFFORT",
"ENCORE",
"ENDIVE",
"ENFANT",
"ENTIER",
"ENTRER",
"ENVERS",
"FACILE",
"FARINE",
"FERMER",
"FEUTRE",
"FIGURE",
"FLAMME",
"FLAQUE",
"FLOCON",
"FOURMI",
"FRAISE",
"FRANGE",
"GAGNER",
"GARAGE",
"GARDER",
"GAUCHE",
"GENTIL",
"GIRAFE",
"GOURDE",
"GOUTTE",
"GRAINE",
"GRIFFE",
"GROTTE",
"HANCHE",
"HUMEUR",
"HUMIDE",
"HURLER",
"IMITER",
"INTRUS",
"JALOUX",
"JAMAIS",
"JAMBES",
"JAMBON",
"JARDIN",
"JOYEUX",
"JUMEAU",
"LAISSE",
"LANCER",
"LANGUE",
"LAVABO",
"LETTRE",
"LOUCHE",
"MADAME",
"MAIGRE",
"MAISON",
"MALADE",
"MANCHE",
"MANGER",
"MARCHE",
"MARIER",
"MARRON",
"MASQUE",
"MENTON",
"METTRE",
"MEUBLE",
"MILIEU",
"MINUIT",
"MINUTE",
"MOMENT",
"MONTER",
"MONTRE",
"MOQUER",
"MORDRE",
"MOTEUR",
"MOUCHE",
"MOUFLE",
"MOULIN",
"MOURIR",
"MOUSSE",
"MOUTON",
"MUGUET",
"MUSCLE",
"NAVIRE",
"NEIGER",
"NOMBRE",
"OFFRIR",
"OIGNON",
"OISEAU",
"ORANGE",
"OUVRIR",
"PAILLE",
"PALAIS",
"PANIER",
"PAPIER",
"PAQUET",
"PARDON",
"PAREIL",
"PARENT",
"PARFUM",
"PARLER",
"PARTIE",
"PARTIR",
"PASSER",
"PEIGNE",
"PENSER",
"PERCER",
"PERDRE",
"PERSIL",
"PHOQUE",
"PIERRE",
"PIGEON",
"PILOTE",
"PINCER",
"PIQUER",
"PLAIRE",
"PLANTE",
"PLIAGE",
"POINTE",
"POINTU",
"POISON",
"POLICE",
"PORTER",
"POSTER",
"POTAGE",
"POULET",
"PRINCE",
"PRISES",
"PRIVER",
"PROPRE",
"PUZZLE",
"PYJAMA",
"QUATRE",
"RAISIN",
"RAMPER",
"RANGER",
"RAPIDE",
"RASOIR",
"RAYURE",
"REGARD",
"REMUER",
"RENARD",
"RENDRE",
"REQUIN",
"RESTER",
"RETARD",
"RETOUR",
"REVOIR",
"RIDEAU",
"ROCHER",
"RONGER",
"ROULER",
"SAISON",
"SALADE",
"SALUER",
"SAMEDI",
"SAUTER",
"SAUVER",
"SAVOIR",
"SENTIR",
"SERRER",
"SERVIR",
"SIESTE",
"SOLDAT",
"SOLEIL",
"SOLIDE",
"SOMBRE",
"SOMMET",
"SONNER",
"SORTIE",
"SORTIR",
"SOUPLE",
"SOURIS",
"SQUARE",
"SUIVRE",
"AMUSER",
"TAILLE",
"TAMPON",
"TENDRE",
"TICKET",
"TIMBRE",
"TIROIR",
"TOMATE",
"TOMBER",
"TORTUE",
"TRAIRE",
"TRICOT",
"TRISTE",
"TROUER",
"TULIPE",
"TUNNEL",
"VALISE",
"VENDRE",
"VENTER",
"VENTRE",
"VERSER",
"VIANDE",
"VIRAGE",
"VISAGE",
"VIVANT",
"VOISIN",
"VOYAGE",
"YAOURT",
"ZIGZAG",
],
[
"ABEILLE",
"ABRICOT",
"ABRITER",
"ACHETER",
"ADRESSE",
"AFFAIRE",
"AFFICHE",
"AJOUTER",
"ALIMENT",
"ALLUMER",
"AMPOULE",
"AMUSANT",
"ANIMAUX",
"APPELER",
"APPUYER",
"ARMOIRE",
"ARRIVER",
"ARROSER",
"AVANCER",
"BAGARRE",
"BAIGNER",
"BAISSER",
"BALAYER",
"BALEINE",
"BARREAU",
"BASSINE",
"BIBERON",
"BIFTECK",
"BISCUIT",
"BIZARRE",
"BLESSER",
"BLOTTIR",
"BOISSON",
"BOUCHER",
"BOUCHON",
"BOUQUET",
"BRANCHE",
"BRILLER",
"BUISSON",
"CABINET",
"CADENAS",
"CAGOULE",
"CAILLOU",
"CAPRICE",
"CARESSE",
"CAROTTE",
"CARREAU",
"CERCEAU",
"CHAISES",
"CHAMBRE",
"CHANGER",
"CHANSON",
"CHANTER",
"CHAPEAU",
"CHARGER",
"CHARIOT",
"CHASSER",
"CHEMISE",
"CHIFFON",
"CHIFFRE",
"CHOISIR",
"CIGOGNE",
"CISEAUX",
"CLOCHER",
"COCOTTE",
"COFFRET",
"COIFFER",
"COLLANT",
"COLLIER",
"COLLINE",
"COMPTER",
"CONTENT",
"CORBEAU",
"COUCHER",
"COUETTE",
"COULEUR",
"COULOIR",
"COUSINE",
"COUSSIN",
"COUTEAU",
"COUVERT",
"COUVRIR",
"CRACHER",
"CRAPAUD",
"CRAVATE",
"CREUSER",
"CROCHET",
"CROISER",
"CROQUER",
"CUISINE",
"CULOTTE",
"CURIEUX",
"CUVETTE",
"DAUPHIN",
"DERNIER",
"DESSERT",
"DOCTEUR",
"DOSSIER",
"DOUCHER",
"DRAPEAU",
"EFFACER",
"EMMENER",
"ENDROIT",
"ENFILER",
"ENLEVER",
"ENNUYER",
"ENVOLER",
"ENVOYER",
"ESSAYER",
"ESSENCE",
"ESSUYER",
"EXCUSER",
"FACTEUR",
"FALLOIR",
"FATIGUE",
"FERMIER",
"FEUILLE",
"FICELLE",
"FLOTTER",
"FRAPPER",
"FROMAGE",
"FROTTER",
"GALETTE",
"GALOPER",
"GARDIEN",
"GLISSER",
"GOBELET",
"GONFLER",
"GOUTTES",
"GRAINES",
"GRANDIR",
"GRATTER",
"GRIFFER",
"GRILLER",
"GRIMACE",
"GRIMPER",
"GRONDER",
"GUETTER",
"HABITER",
"HAMSTER",
"HARICOT",
"HEUREUX",
"HORLOGE",
"IMMENSE",
"INONDER",
"INSECTE",
"INVITER",
"JONGLER",
"LAISSER",
"LECTURE",
"LESSIVE",
"LIQUIDE",
"MACHINE",
"MAGASIN",
"MAILLOT",
"MALADIE",
"MANQUER",
"MANTEAU",
"MARCHER",
"MARIAGE",
"MARTEAU",
"MATELAS",
"MAUVAIS",
"MESURER",
"MOINEAU",
"MONNAIE",
"MONTRER",
"MORCEAU",
"MOUCHER",
"MOUETTE",
"MUSIQUE",
"NOURRIR",
"NOUVEAU",
"NUAGEUX",
"OBLIGER",
"OCCUPER",
"OREILLE",
"OUBLIER",
"OURAGAN",
"OUVRIER",
"PANNEAU",
"PARASOL",
"PARKING",
"PASSAGE",
"PATIENT",
"PAYSAGE",
"PEINDRE",
"PELUCHE",
"PENCHER",
"PENDULE",
"PERCHER",
"PINCEAU",
"PISCINE",
"PLACARD",
"PLAFOND",
"PLANCHE",
"PLANTER",
"PLATEAU",
"PLEURER",
"PLONGER",
"POIGNET",
"POIREAU",
"POISSON",
"POMPIER",
"POUSSER",
"POUSSIN",
"POUVOIR",
"PRAIRIE",
"PREMIER",
"PRENDRE",
"PRESQUE",
"PRESSER",
"PROFOND",
"PRUDENT",
"QUITTER",
"RECETTE",
"RECULER",
"REFUSER",
"REMPLIR",
"RENTRER",
"REPOSER",
"REVENIR",
"ROBINET",
"RONFLER",
"ROULADE",
"ROULEAU",
"RUGUEUX",
"SAIGNER",
"SARDINE",
"SECOUER",
"SEMAINE",
"SEMELLE",
"SERPENT",
"SERRURE",
"SIFFLER",
"SIFFLET",
"SILENCE",
"SOIGNER",
"SOMMEIL",
"SOURCIL",
"SOURIRE",
"SOUVENT",
"SUIVANT",
"TABLEAU",
"TABLIER",
"TAILLER",
"TAMBOUR",
"TARTINE",
"TERRAIN",
"TERRIER",
"TORCHON",
"TOUCHER",
"TOURNER",
"TOUSSER",
"TRANCHE",
"TRAVAIL",
"TRAVAUX",
"TREMPER",
"TRICHER",
"TROUSSE",
"TROUVER",
"UNIVERS",
"VILLAGE",
"VISITER",
"VITESSE",
"VOITURE",
"VOULOIR",
"VOYAGER",
],
[
"ACCIDENT",
"ACROBATE",
"AIGUILLE",
"ALLONGER",
"ALPHABET",
"APPAREIL",
"APPORTER",
"AQUARIUM",
"ARRACHER",
"ARROSOIR",
"ASSIETTE",
"ATTACHER",
"ATTENDRE",
"ATTERRIR",
"ATTRAPER",
"BAGARRER",
"BAGUETTE",
"BALANCER",
"BARBOTER",
"BARRETTE",
"BEAUCOUP",
"BONHOMME",
"BOUILLIR",
"BOURGEON",
"BOUTIQUE",
"BRACELET",
"BRETELLE",
"BROUETTE",
"CAMARADE",
"CAMPAGNE",
"CANIVEAU",
"CARAVANE",
"CARESSER",
"CARTABLE",
"CASSETTE",
"CEINTURE",
"CHASSEUR",
"CHAUFFER",
"CHAUSSER",
"CHAUSSON",
"CHENILLE",
"CHERCHER",
"CHEVILLE",
"CHOCOLAT",
"CHOUETTE",
"COIFFEUR",
"COLORIER",
"COMPARER",
"CONDUIRE",
"CONSOLER",
"COQUILLE",
"COURONNE",
"COURRIER",
"CREVETTE",
"CUEILLIR",
"CUISINER",
"DEMANDER",
"DENTISTE",
"DESSINER",
"DIMANCHE",
"DISCUTER",
"DISPUTER",
"DOUILLET",
"EMPORTER",
"ENCEINTE",
"ENDORMIR",
"ENFERMER",
"ENFONCER",
"ENSEMBLE",
"ENTENDRE",
"ENTERRER",
"ENTOURER",
"ESCABEAU",
"ESCALADE",
"ESCALIER",
"ESCARGOT",
"FAUTEUIL",
"FONTAINE",
"FOUILLER",
"GOURMAND",
"HABILLER",
"HISTOIRE",
"IMMEUBLE",
"IMMOBILE",
"INCENDIE",
"INSTABLE",
"JARDINER",
"JOURNAUX",
"JUMELLES",
"LUNETTES",
"MAGAZINE",
"MAGICIEN",
"MARCHAND",
"MEILLEUR",
"MENSONGE",
"MERCREDI",
"MONSIEUR",
"MONTAGNE",
"MONUMENT",
"MOUCHOIR",
"MOUILLER",
"NETTOYER",
"NOISETTE",
"OREILLER",
"PANTALON",
"PAPILLON",
"PARCOURS",
"PARTAGER",
"PATAUGER",
"PEINTURE",
"PERSONNE",
"PLAINDRE",
"PLEUVOIR",
"PLUVIEUX",
"POLICIER",
"POUBELLE",
"PROMENER",
"QUARTIER",
"QUESTION",
"RACONTER",
"RAMASSER",
"RAPPELER",
"RAQUETTE",
"RECEVOIR",
"RECOUDRE",
"REGARDER",
"REMETTRE",
"REPASSER",
"RESPIRER",
"RETARDER",
"RONDELLE",
"SALADIER",
"SONNETTE",
"SOUFFLER",
"SOUFFRIR",
"SOULEVER",
"SURPRISE",
"TABOURET",
"TERMINER",
"TERRIBLE",
"TOBOGGAN",
"TOILETTE",
"TONNERRE",
"TOUJOURS",
"TRACTEUR",
"TREMBLER",
"TRICOTER",
"TRICYCLE",
"TROTTOIR",
"UNIFORME",
"VACANCES",
"VENDREDI",
],
[
"ACCOUCHER",
"ACCROCHER",
"ALLUMETTE",
"AMBULANCE",
"ASCENSEUR",
"ATTENTION",
"BOUCHERIE",
"BOULANGER",
"BOUSCULER",
"BOUTEILLE",
"BRICOLAGE",
"CARREFOUR",
"CASQUETTE",
"CASSEROLE",
"CATALOGUE",
"CAUCHEMAR",
"CHANGEANT",
"CHAUSSURE",
"CHUCHOTER",
"CIGARETTE",
"CLIGNOTER",
"COLORIAGE",
"COMMENCER",
"CONCOMBRE",
"CONFITURE",
"CONTINUER",
"CONTRAIRE",
"COQUETIER",
"CORBEILLE",
"COUVERCLE",
"CROCODILE",
"DANGEREUX",
"DESCENDRE",
"DIFFICILE",
"DINOSAURE",
"DIRECTEUR",
"EMBRASSER",
"ENTONNOIR",
"ENVELOPPE",
"ESCALADER",
"EXPLIQUER",
"FABRIQUER",
"FLEURISTE",
"FRAMBOISE",
"GRIGNOTER",
"GUIRLANDE",
"IMPORTANT",
"INFIRMIER",
"INSTALLER",
"JONQUILLE",
"KANGOUROU",
"MALADROIT",
"MAQUILLER",
"MOUSTACHE",
"MOUSTIQUE",
"ORCHESTRE",
"PANSEMENT",
"PARAPLUIE",
"PERROQUET",
"PHARMACIE",
"PLASTIQUE",
"PLONGEOIR",
"POUSSETTE",
"PRINCESSE",
"PROMENADE",
"PROMETTRE",
"RADIATEUR",
"REMERCIER",
"REMPLACER",
"RENVERSER",
"RETOURNER",
"RETROUVER",
"SERVIETTE",
"SHAMPOING",
"SOUHAITER",
"SOULIGNER",
"SPECTACLE",
"SQUELETTE",
"TOURNEVIS",
"TRAVERSER",
"TROMPETTE",
"VAISSELLE",
"XYLOPHONE",
],
[
"ASPIRATEUR",
"BICYCLETTE",
"BROUILLARD",
"CALENDRIER",
"CHAMPIGNON",
"CHAUSSETTE",
"CITROUILLE",
"COCCINELLE",
"CONSTRUIRE",
"COQUELICOT",
"COQUILLAGE",
"COUVERTURE",
"CROCODILES",
"DENTIFRICE",
"DIRECTRICE",
"DISTRIBUER",
"FOURCHETTE",
"GRENOUILLE",
"HIRONDELLE",
"IMPOSSIBLE",
"INSTRUMENT",
"INVITATION",
"MAINTENANT",
"MAQUILLAGE",
"MARGUERITE",
"MATERNELLE",
"NOURRITURE",
"ORDINATEUR",
"ORDONNANCE",
"PASSERELLE",
"PHARMACIEN",
"POURSUIVRE",
"RENCONTRER",
"RESSEMBLER",
"RESTAURANT",
"SURVEILLER",
"TRAMPOLINE",
"TRANQUILLE",
"TRANSPIRER",
"TRAVAILLER",
]
];

export default function () {

    // TODO choose ciphers and plain word.
    var minLength = words[0][0].length;
    var cipherLengths = [
      [7, 10, 8, 7],
      [10, 9, 9, 4],
      [5, 10, 7, 10],
      [4, 5, 6, 4, 5, 6]
    ];

    var ciphers = [];
    var allWords = [];
    for (var iCipher = 0; iCipher < cipherLengths.length; iCipher++) {
       var cipher = "";
       for (var iWord = 0; iWord < cipherLengths[iCipher].length; iWord++) {
          if (iWord != 0) {
             cipher += " ";
          }
          var length = cipherLengths[iCipher][iWord];
          var iChoice = Math.trunc(Math.random() * words[length - minLength].length);
          var word = words[length - minLength][iChoice];
          cipher += word;
          if (length == 7) {
             allWords.push(word);
          }
       }
       ciphers.push(cipher);
    }
    var plainWord = allWords[Math.trunc(Math.random() * allWords.length)];
/*
    var ciphers = [
      "RSJIZL XRBA FBCSWO CNRWGS",
      "NLB OQH KXGSR EGNVAE JRWS",
      "Z Y X W V U T S R Q P O N"
    ];
    var plainWord = "QUART";
*/
    // TODO choose key and use it.
    var secretKey = [];
    for (var iKey = 0; iKey < ciphers[0].length; iKey++) {
       secretKey.push(Math.trunc(Math.random() * 26));
    }
  
    for (var iCipher = 0; iCipher < ciphers.length; iCipher++) {
       var newCipher = "";
       for (var iLetter = 0; iLetter < secretKey.length; iLetter++) {
          var letter = ciphers[iCipher][iLetter];
          if ((letter == ' ') || (letter == '_')) {
             newCipher += ' ';
          } else {
             var rank = letter.charCodeAt(0) - "A".charCodeAt(0);
             rank = (rank - secretKey[iLetter] + 26) % 26;
             newCipher += String.fromCharCode(rank + "A".charCodeAt(0));
          }
       }
       ciphers[iCipher] = newCipher;
    }
    // TODO hints.
    var task = {ciphers, plainWord, hints: {}};
    var full_task = Object.assign({secretKey: secretKey}, task);

    return {task, full_task};

};
